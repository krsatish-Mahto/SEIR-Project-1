...
stop_word={"the", "of", "is", "in", "on", "an", "a","this", "but", "from", "at", "on",
           "they", "their", "here", "there", "it", "with"}

def word_frequency(url):
    identity={"User-Agent": "Mozilla/5.0"}
    response= requests.get(url, headers=identity,timeout=10)
...
def calc_polynomial_rolling_hash_fn(word):
    ...
    for character in word:
        ...
        # print(hash_value)
        power=(power*p)%2**64
    # print(f"Hash value of {word}:", hash_value)
    return hash_value
